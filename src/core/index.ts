...
export default class CorePlugin {
  ...
  mergeConfig(config:WebpackOptionsNormalized){
    ...
    // 端口
    if(port) config.devServer.port = port
    // 启动时打开浏览器
    if(open) config.devServer.open = open
    if(publicPath) config.output.publicPath = publicPath
    if(outputDir) config.output.path = resolve(process.cwd(), outputDir)
    if(proxy) config.devServer.proxy = proxy
    if(webpackPlugins) config.plugins.push(...webpackPlugins)
    config.module.rules.push(...rules)
    if(!config.resolve.extensions) config.resolve.extensions = []
    config.resolve.extensions.push(...extensions)
    if(!config.resolve.mainFiles) config.resolve.mainFiles = []
    config.resolve.mainFiles.push('index')
    config.devtool = config.mode === 'development' ? 'source-map' : false
    if(webpack && typeof webpack === 'function') webpack(config)
    config.watch = true
  ...
  laodPlugins(){
    this.userConfig.plugins?.forEach(plugin => {
      ...
      if (runtime) this.runtimes.push(runtime)
      setup?.({
        context,
  ...
  /**修改用户配置 */
  modifyUserConfig:PluginOptions['modifyUserConfig'] = (fn) =>{
    this.userConfig = fn(this.userConfig)
  }
  /**添加文件 */
  addFile:PluginOptions['addFile'] = ({ content, outPath }) => {
    writeFileSync(outPath, content)
  }
  /**根据Handlebars模板写入文件 */
  addFileTemplate: PluginOptions['addFileTemplate'] = (options) => {
    renderHbsTpl(options)
  }
  /**添加额外的pageConfig的配置类型 */
  addPageConfigType: PluginOptions['addPageConfigType'] = (options) => {
    this.pageConfigTypes.push(options)
  }
  /**添加额外的appConfig的配置类型 */
  addAppConfigType: PluginOptions['addAppConfigType'] = (options) => {
    this.appConfigTypes.push(options)
  }
  /**添加从ran命名空间导出的模块 */
  addExport: PluginOptions['addExport'] = (options) => {
    this.exports.push(options)
  }
  /**在入口文件的最前面添加import */
  addEntryImport: PluginOptions['addEntryImport'] = (options) => {
    this.imports.push(options)
  }
  /**在入口文件的最前插入代码 */
  addEntryCodeAhead: PluginOptions['addEntryCodeAhead'] = (code) => {
    this.aheadCodes.push(code)
  }
  /**在入口文件的最后插入代码 */
  addEntryCodeTail: PluginOptions['addEntryCodeTail'] = (code) => {
